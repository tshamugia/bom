...
import { items, vendors, categories, projects, bomRevisions, bomLines, bomSections, user } from "@/db/schema";
...
async function seed() {
  ...
  await db.insert(user).values({ id: "u1", name: "T", email: `u1-${org.id}@test.local` });
